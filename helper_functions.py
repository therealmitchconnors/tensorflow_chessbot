...
def loadImageFromURL(img_url):
  ...
  con = urlopen(req)
  return tf.image.decode_image(con.read(), name="rawimage")

def loadImageFromPath(img_path):
  """Load PIL image from image filepath, keep as color"""
  return tf.image.decode_image(open(img_path,'rb'))
...
def loadImgur(image_url):
  ...
  soup = BeautifulSoup(requests.get(image_url).content, "html5lib")
  # Get metadata tags
